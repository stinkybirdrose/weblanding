...
class Api extends \ShopEngine\Base\Api {
	...
	public function get_posts() {
		...
		$posts = $wpdb->get_results( $wpdb->prepare("SELECT * FROM $wpdb->posts WHERE post_type=%s AND post_status='publish' AND post_title LIKE %s LIMIT 10", sanitize_text_field($data['post_type']),  '%'. $wpdb->esc_like( $search ) .'%') );
